...
**Linchpin for host interop.** `wild { name(args) }` type-checks inside `@std-sys` + `!{ffi}` and elaborates to `Node::Op { prim: "wild:…" }`, but the host-call registry is **empty by design** (RFC-0028 §4.3) — see `docs/GAP-ffi-host-and-surface.md`. That is an **evaluation / missing host prim** gap, not a surface gap.
...
| 9 | `wild { name(args) }` host-call form | RFC-0028; ADR-014; M-661 | Y | Y | Y‡ | **R**§ | **R**§ | `elab_wild` → `wild:name`; §empty host registry — `docs/GAP-ffi-host-and-surface.md` |
...
§ Runtime: no `wild:` prims registered by default.  
...
| G15 | `wild` host ops actually run | RFC-0028 §4.3; GAP note | **evaluation** + **missing prim** | Empty `wild:` registry by design | GAP-ffi-host-and-surface.md; elab_wild doc |
